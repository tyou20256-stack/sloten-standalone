...
### 1.4 GAS Webhook URL の事前検証 ★★★

**GAS 連携を使っているプロジェクトでは必須**。詳細は [`08-gas-urls.md`](08-gas-urls.md) 参照。
...
# 2. 5 本の URL が期待通り保存されていることを確認:
#    GAS_BOT_WEBHOOK_URL / BANK_TRANSFER_BOT_WEBHOOK_URL / EC_DEPOSIT_BOT_WEBHOOK_URL
#    BONUS_CODE_WEBHOOK_URL / OPERATOR_ATTACHMENT_WEBHOOK_URL

# 3. isSafeOutboundUrl で各 URL が通ることを確認
cd /c/Users/PC/OneDrive/Desktop/sloten-standalone-overnight-2026-04-17-2311
node --input-type=module -e "
import { isSafeOutboundUrl } from './src/safe-url.mjs';
const env = { ALLOWED_WEBHOOK_HOSTS: 'script.google.com,sloten.io' };  // §4 で設定予定なら
['<URL1>', '<URL2>', '<URL3>', '<URL4>', '<URL5>'].forEach(u =>
...
**✗ が出る場合の対応**:
- `script.google.com` 以外のホストが含まれる → `ALLOWED_WEBHOOK_HOSTS` に追加 (§4.1) または設定をスキップ
- private IP や不正 scheme が保存されている → 本番 URL が誤って保存されている可能性あり、要調査
...
### 8.6 GAS Webhook URL 疎通 (GAS 連携ある場合 ★★★)

詳細は [`08-gas-urls.md`](08-gas-urls.md) §D 参照。

- [ ] 管理画面「GAS URL」タブで **5 本** とも ping 成功 (2xx + body preview)
  - [ ] `GAS_BOT_WEBHOOK_URL`
  - [ ] `BANK_TRANSFER_BOT_WEBHOOK_URL`
  - [ ] `EC_DEPOSIT_BOT_WEBHOOK_URL`
  - [ ] `BONUS_CODE_WEBHOOK_URL`
  - [ ] `OPERATOR_ATTACHMENT_WEBHOOK_URL`
- [ ] canary トラフィックでボーナスコード送信 → `bonus_code_submissions.gas_forwarded=1` 確認
- [ ] Worker ログに `[bonus-codes] GAS forward skipped: unsafe URL rejected` が **出ていない**

ping 失敗時の対応:
- 400 "URL no longer passes safety check" → `ALLOWED_WEBHOOK_HOSTS` 要見直し
- 404 → GAS script 再デプロイの可能性、新 URL を管理画面で保存
- 5xx / timeout → GAS 側の問題、本サービス関係なし
